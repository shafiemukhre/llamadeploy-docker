...
class OpenSourceRAG(Workflow):
    ...
    async def prepare_for_retrieval(
        ...
        # this is correct, use the HF_LLM_URL
        model_url = "https://pxtmbr01un274ha7.us-east-1.aws.endpoints.huggingface.cloud"

        query_str: str | None = ev.query_str
        ...
        await ctx.set("rag_pipeline", QueryPipeline(chain=[DEFAULT_RAG_PROMPT, llm]))
    ...
    async def augment_and_generate(self, ctx: Context, ev: RetrieveEvent) -> StopEvent:
        ...
        relevancy = relevancy_pipeline.run(context=relevant_text, question=query_str)
...
async def main():
    print(
        # await open_source_rag_wf.run(query_str="What state is the complaint filed in?")
        await open_source_rag_wf.run(query_str="List all LLCs that Elon Musk is suing")
    )
